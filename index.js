...
function getWinnersByYear(data, getYearscb, GetWinnercb) {
  const dataWinners = GetWinnercb(data, getFinals);
  const dataYears = getYearscb(data, getFinals);
  return dataWinners.map(function (item, index) {
    return `In ${dataYears[index]}, ${item} won the world cup!`;
  });
}
...
function getAverageGoals(getFinalscb) {
  ...
  return (goals / getFinalscb.length).toFixed(2);
}
